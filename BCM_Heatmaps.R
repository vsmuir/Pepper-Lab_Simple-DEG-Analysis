...
########################################
normCounts_all <- counts_batch_corr5
normCounts_IgD <- counts_batch_corr5[,design$Ig == "D"]
normCounts_IgM <- counts_batch_corr5[,design$Ig == "M"]
normCounts_IgG <- counts_batch_corr5[,design$Ig == "G"]
design_all <- design
...
DE_IgD <- union(rownames(less_sig_genes_DvM5), rownames(less_sig_genes_DvG5))
DE_IgM <- union(rownames(less_sig_genes_DvM5), rownames(less_sig_genes_MvG5))  
DE_IgG <- union(rownames(less_sig_genes_DvG5), rownames(less_sig_genes_MvG5)) 
DE_all <- union(rownames(less_sig_genes_DvG5),union(rownames(less_sig_genes_DvM5), rownames(less_sig_genes_MvG5)))
...
M_up_vs_DG <- intersect(rownames(less_sig_genes_MvG5[less_sig_genes_MvG5$logFC > 0,]),
                        rownames(less_sig_genes_DvM5[(less_sig_genes_DvM5$logFC < 0),]))
MG_up_vs_D <- intersect(rownames(less_sig_genes_DvG5[(less_sig_genes_DvG5$logFC < 0),]),
                        rownames(less_sig_genes_DvM5[(less_sig_genes_DvM5$logFC < 0),]))
M_up_vs_D <- setdiff(rownames(less_sig_genes_DvM5[(less_sig_genes_DvM5$logFC < 0),]), union(MG_up_vs_D, M_up_vs_DG))
G_up_vs_D <- setdiff(rownames(less_sig_genes_DvG5[(less_sig_genes_DvG5$logFC < 0),]), MG_up_vs_D)
M_down_vs_D <- rownames(less_sig_genes_DvM5[(less_sig_genes_DvM5$logFC > 0),])
G_down_vs_D <- rownames(less_sig_genes_DvG5[(less_sig_genes_DvG5$logFC > 0),])
...
up_in_D <- ifelse(rownames(norm_DE_counts_all) %in% union(rownames(less_sig_genes_DvG5[(less_sig_genes_DvG5$logFC > 0),]),
                                                         rownames(less_sig_genes_DvM5[(less_sig_genes_DvM5$logFC > 0),])), 
                                                        "lightpink1", 0)
up_in_M <-ifelse(rownames(norm_DE_counts_all) %in% union(rownames(less_sig_genes_MvG5[less_sig_genes_MvG5$logFC > 0,]),
                                                         rownames(less_sig_genes_DvM5[(less_sig_genes_DvM5$logFC < 0),])), 
                                                        "skyblue3", 0)
up_in_G <-ifelse(rownames(norm_DE_counts_all) %in% union(rownames(less_sig_genes_MvG5[less_sig_genes_MvG5$logFC < 0,]),
                                                         rownames(less_sig_genes_DvG5[(less_sig_genes_DvG5$logFC < 0),])), 
                                                        "palegreen", 0)
...
pdf(file="Limma/With MedCV/Heatmaps/IgComp_1.pdf")
heatmap.2(norm_DE_counts_all, dendrogram = "none", scale="none", col=my.col,
          ...
          key=TRUE,density.info="none", margins = c(7,9), main = "Plasmodium-specific Memory B Cells", xlab = "Ig")
dev.off()
...
pdf(file="Limma/With MedCV/Heatmaps/IgComp_byIg.pdf")
...
pdf(file="Limma/With MedCV/Heatmaps/IgComp_byMouse.pdf")
heatmap.2(normCounts_byIg, dendrogram = "none", scale="none", col=my.col, keysize = 1,
          ...
          key=TRUE,density.info="none", margins = c(7,9), main = "Plasmodium-specific Memory B Cells", xlab = "Mouse")
dev.off()
